...
import { modelData, setParmsandQueryEraseModel, setParmsandQueryModel } from "./components/helpers/modelAPI";
...
const App = () => {
  ...
  const [multiMaskModel, setMultiMaskModel] = useState<InferenceSession | null>(null);
  ...
  const runMultiMaskModel = async () => {
    try {
      ...
      allMasksSorted = allMasksSorted.filter((obj: any, i: number) => keepArray[i]);
      allUncertainIoUSorted = allUncertainIoUSorted.filter((obj: any, i: number) => keepArray[i]);
  ...
  const handleSelectedImage = async (data: File | URL, options?: { shouldNotFetchAllModel?: boolean; shouldDownload?: boolean }) => {
    try {
  ...
  const handleAllModelResults = ({
    ...
    const allMaskSVG = allJSON.map(
      (el: { encodedMask: string; bbox: number[]; score: number; point_coord: number[]; uncertain_iou: number; area: number }) => {
    ...
    <>
      ...
      <Routes>
        ...
        <Route
          path="/terms"
          element={
            <div className={`flex flex-col h-full w-full overflow-x-hidden items-center overflow-y-scroll`}>
