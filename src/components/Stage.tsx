...
import { AnnotationProps, modelInputProps, StageProps } from "./helpers/Interface";
...
const Stage = ({
  ...
  const [allText, setAllText] = useState<string | React.DOMElement<React.DOMAttributes<HTMLElement>, HTMLElement>>("");

  useEffect(() => {
    if (isStandalone && model) {
      const url = new URL(HOMEPAGE_IMAGE, window.location.origin);
  ...
  const handleCreateSticker = () => {
    ...
    superDefer(() =>
      superDefer(() =>
        superDefer(() =>
          superDefer(() => {
            createCutouts(image!, svg!, allsvg!, stickers, setStickers, segmentTypes, setActiveSticker, setIsLoading, scale!, handleResetInteraction);
          })
        )
      )
    );
  ...
  const getClick = (e: any, x: number, y: number): modelInputProps | undefined => {
    let clickType;
  ...
  const handleSegmentByBox = ({ sx, sy, x, y }: Points, extraClick?: modelInputProps, newerClicks?: modelInputProps[]) => {
  ...
  const adjustPointsToRange = (points: Points, extraClick?: modelInputProps, newClicks?: modelInputProps[]) => {
    const range = findClickRange(extraClick, newClicks);
    if (!range || !range.xMin || !range.yMin || !range.xMax || !range.yMax) return;
  ...
  const findClickRange = (extraClick?: modelInputProps, newClicks?: modelInputProps[]) => {
    let xMin;
    let yMin;
    let xMax;
    let yMax;
  ...
  const handleRedoInteraction = () => {
    if (clicksHistory?.length && prevAnnotaiton?.length && segmentTypes === "Box") {
        ...
        <>
          {image && (
            <div
              ...
              ref={containerRef}
              style={window.innerWidth < 768 ? { height: "calc(100vh/2" } : { height: canvasHeight }}
            >
              <Canvas
                konvaRef={konvaRef}
              ...
              <div
                className={`absolute top-0 right-0 bottom-0 left-0 bg-opacity-70 bg-black flex items-center justify-center text-white text-lg font-bold transition-opacity ${
                  shouldShowHomepageOverlay ? "opacity-100" : "opacity-0 pointer-events-none"
                }`}
                id="canvas-overlay"
              >
                <FeatureSummary actions={[{ action: "Demo", actionUrl: "/demo" }]} darkMode centerAlign>
        ...
        <>
          {image || isToolBarUpload ? (
            <div className="flex items-stretch justify-center flex-1 overflow-hidden stage">
              {showLoadingModal ? <LoadingModal handleResetState={handleResetState} /> : <PointsModal />}
              <SegmentDrawer
              ...
              <div className="relative flex flex-col items-center md:justify-center flex-1 overflow-hidden md:overflow-visible md:px-12 md:py-9">
                ...
                <div className="relative w-full h-[calc(100%-6rem)] md:h-full md:my-7" ref={containerRef}>
                  <Profiler
                    id="Canvas"
                    onRender={(
            ...
            <div className="flex items-stretch justify-center flex-1 overflow-hidden stage">
              <ImagePicker handleSelectedImage={handleSelectedImage} showGallery={[showGallery, setShowGallery]} />
            </div>
          ) : (
            <></>
          )}
