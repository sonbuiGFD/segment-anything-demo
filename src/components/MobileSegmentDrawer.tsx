...
const MobileSegmentDrawer = ({
  ...
  return (
    ...
    >
      <div {...handlers}>
        <div className="inline-block w-24 h-1 bg-gray-200 rounded-full" onClick={() => setStickerTabBool(!stickerTabBool)}></div>
        <div className="flex w-[18rem] gap-4 mt-2 font-bold mx-auto">
          <button
            className={`flex flex-1 flex-row items-center px-2 py-1 text-sm justify-center ${
              userNegClickBool ? "text-gray-700 bg-gray-400" : "text-white bg-blue-700"
            } rounded-xl w-max`}
            onClick={() => setUserNegClickBool(false)}
          >
            <svg className="w-6 h-4" fill="none" viewBox="0 0 24 24" stroke={`${userNegClickBool ? "#1C2B33" : "#fff"}`}>
          ...
          <button
            className={`flex flex-1 flex-row items-center px-2 py-1 text-sm justify-center rounded-xl w-max ${
              userNegClickBool ? "text-white bg-blue-700" : "text-gray-700 bg-gray-400"
        ...
        <ul className="flex w-screen mb-2 font-semi-bold justify-evenly text-secondary">
          <li>
            <a
              className={`flex flex-col items-center pt-2 ${isModelLoaded.allModel === false && "opacity-70 pointer-events-none"}`}
              onClick={() => {
            ...
            >
              ...
              <span className={(!isModelLoaded.allModel ? "opacity-60" : "") + (everythingBool ? "text-blue-700" : "#000")}>Everything</span>
            </a>
          </li>
          {/* <li>
          <a
          ...
          <li>
            ...
            >
              <svg className="w-5 m-1" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
              ...
              <span className={stickerTabBool ? "text-blue-700" : ""}>Cut-outs</span>
            </a>
          </li>
        </ul>
      </div>
      <div className="absolute w-full bg-white h-[17rem] mt-[-1px]">
        <div className="h-full p-5 pb-8 overflow-auto bg-white">
          <Button
            size="md"
            className={`flex flex-row-reverse normal-case border-none mt-5 mx-auto bg-gray-300 ${!everythingBool && !svg && "disabled"}`}
            onClick={(e) => {
            ...
            {stickers.map((el: HTMLCanvasElement, i) => (
              <img
                ...
                className={`m-5 max-w-[75%] max-h-20 md:max-h-24 lg:max-h-28 xl:max-h-32 cursor-pointer inline hover:opacity-100 ${
                  stickers.length === 1 || i === activeSticker ? "sticker-select" : ""
                }`}
                alt="cutout"
                src={el.toDataURL()}
