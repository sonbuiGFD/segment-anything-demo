...
const Canvas = ({
  ...
  useEffect(() => {
    if (segmentTypes === "All") {
      ...
      const animateAllSvg: Konva.Layer | undefined = konvaRef!.current!.findOne(".animateAllSvg");
    ...
    const load = async () => {
      try {
        ...
        allsvg.map(async ({ svg, point_coord }, i) => {
          ...
          const newAllTimeout3 = setTimeout(() => {
            ...
            setAllText(
              <span>
                ...
                <a href="https://ai.facebook.com/research/publications/segment-anything/" target={"_blank"} className="underline">
                  Paper
                </a>
                ,{" "}
                <a href="https://ai.facebook.com/blog/segment-anything-foundation-model-image-segmentation/" target={"_blank"} className="underline">
                  Blog Post
                </a>
                , or{" "}
                <a href="https://github.com/facebookresearch/segment-anything" target={"_blank"} className="underline">
                  Code
                </a>
                .
              </span>
          ...
          setAllTimeouts((prev) => [...prev, newAllTimeout1, newAllTimeout2, newAllTimeout3]);
        });
      } catch (error) {
        console.log("Rejected");
      }
  ...
  const shouldShowSpinner =
    (segmentTypes === "All" && isLoading) || (isLoading && !hasClicked && !isModelLoaded.boxModel) || (isLoading && isErasing);
      ...
      >
        <div
          ...
          onMouseOut={() => {
            ...
            if (segmentTypes === "Click" && isMultiMaskMode && clicks !== null && clicks.length === 1) {
              setIsHovering(false);
            }
          }}
          className={`Canvas relative ${isLoading ? "pointer-events-none" : ""} ${
            shouldShowAnimation ? "rotate" : isHovering === false ? "unrotate" : ""
        ...
        >
          ...
          <img
            ...
            className={`absolute w-full h-full pointer-events-none ${
              isLoading || (hasClicked && !isMultiMaskMode) || (isMultiMaskMode && clicks) ? "opacity-50" : ""
            }`}
            style={{ margin: 0 }}
          ></img>
          {segmentTypes !== "All" &&
          ...
          {segmentTypes !== "All" && svg && scale && hasClicked && !isMultiMaskMode && (
            <SvgMask xScale={scale.width * scale.uploadScale} yScale={scale.height * scale.uploadScale} svgStr={svg.join(" ")} />
          )}
          <Stage
            className="konva"
            width={canvasDimensions.width}
            ...
            onTouchEnd={(e) => {
              if (stickerTabBool) return;
              if (segmentTypes !== "All" && !hasTouchMoved && numOfTouches === 1) {
                setIsLoading(true);
          ...
          >
            ...
            <Layer name="animateAllSvg">{segmentTypes === "All" && shouldAllAnimate && animateAllSvg()}</Layer>
                  ...
                  return (
                    clickColor && (
                      <Circle
                        ...
                        shadowBlur={5}
                        shadowColor={clickColor === positiveClickColor ? "black" : clickColor}
                        preventDefault={false}
                      />
                    )
                  );
              ...
              {click && clickColor && (
                <>
                  <Circle
                    ...
                    y={(click.y * canvasScale) / scale!.scale}
                    fill={clickColor}
                    shadowColor={clickColor === positiveClickColor ? "black" : negativeClickColor}
                    shadowBlur={5}
                    preventDefault={false}
                    radius={(5 * canvasScale) / scale!.scale}
                  />
          ...
          {segmentTypes !== "All" && maskImg && !hasClicked && (
            <img src={maskImg?.src} style={{ margin: 0 }} className={`absolute top-0 opacity-40 pointer-events-none w-full h-full`}></img>
          )}
          {shouldShowSpinner && (
            <div className={`hidden absolute z-10 md:flex items-center justify-center w-full h-full top-0`}>
              <RadialProgress
                className="animate-spin"
                size={0.3 * Math.min(resizer.scaledWidth, resizer.scaledHeight) + "px"}
                thickness="1rem"
