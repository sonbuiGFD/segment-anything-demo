  ...
  return (
    <div className="flex justify-between w-full p-2 md:hidden">
      <div>
        ...
        <button
          className={`p-3.5 py-2.5 text-sm w-min font-bold bg-white ${(!svg || segmentTypes === "All") && "disabled"}`}
          onClick={handleUndoInteraction}
        >
          Undo
        </button>
        <button
          className={`p-3.5 py-2.5 text-sm font-bold w-min rounded-r-md bg-white ${(!clicksHistory?.length || segmentTypes === "All") && "disabled"}`}
      ...
      >
        <svg className="pr-2" width="24" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
