...
interface SegmentDrawerProps {
  ...
  userNegClickBool: [userNegClickBool: boolean, setUserNegClickBool: (e: boolean) => void];
  showGallery: [showGallery: boolean, setShowGallery: (e: boolean) => void];
  hasClicked: boolean;
  handleSelectedImage: (data: File | URL, options?: { shouldDownload?: boolean; shouldNotFetchAllModel?: boolean }) => void;
}
...
const SegmentDrawer = ({
  ...
  const [visibleStickerHover, setVisibleStickerHover] = useState<boolean>(false);
        ...
        <div className="p-4 pt-5">
          ...
          <div
            ...
            className={`transition-all overflow-hidden pb-2 ${
              segmentTypes !== "Click" && (isClickCollapsed ? "max-h-[40px]" : "max-h-[85px]")
            } px-3 py-2 cursor-pointer rounded-xl ${
              segmentTypes === "Click" ? "outline-blue-700 outline outline-[2.5px]" : "outline outline-gray-200 "
            } ${isCutOut && "hidden"}`}
            onMouseEnter={() => {
              clearTimeout(clickTimeout);
          ...
          >
            <div className="flex">
              <svg width="17" height="24" viewBox="0 0 17 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-3 mr-2">
                <path
              ...
              <span className={`font-bold ${segmentTypes === "Click" && "text-blue-600"}`}>Hover & Click</span>
            </div>
            {segmentTypes !== "Click" && visibleClickHover && (
              <Animate isMounted={isClickMounted}>
                <p className="my-3 text-[11px] opacity-70">Click an object one or more times. Shift-click to remove regions.</p>
              </Animate>
            )}
            {segmentTypes === "Click" && (
              <p className={`my-3 text-[11px] text-blue-700 opacity-70`}>Click an object one or more times. Shift-click to remove regions.</p>
            )}
              ...
              <div onClick={() => setUserNegClickBool(false)} className="flex flex-col items-center">
                <p
                  className={`w-8 h-7 text-3xl leading-7 text-center align-middle rounded-lg mb-1 ${
                    userNegClickBool ? "outline outline-1" : "bg-blue-600 text-white"
                  }`}
                >
                  +
                </p>
                <p className={`text-xs font-bold ${!userNegClickBool && "text-blue-600"}`}>Add Mask</p>
              </div>
              ...
              <div onClick={() => setUserNegClickBool(true)} className={`flex flex-col items-center ${!hasClicked ? "disabled" : ""}`}>
                <p
                  className={`w-8 h-7 text-3xl leading-6 text-center align-middle rounded-lg mb-1 ${
                    userNegClickBool ? "bg-blue-600 text-white" : "outline outline-1"
                  }`}
                >
                  -
                </p>
                <p className={`text-xs font-bold ${userNegClickBool && "text-blue-600"}`}>Remove Area</p>
              </div>
            </div>
            {segmentTypes === "Click" && (
              <SegmentOptions
          ...
          <div
            ...
            className={`transition-all overflow-hidden ${
              segmentTypes !== "Box" && (isBoxCollapsed ? "max-h-[40px]" : "max-h-[85px]")
            } my-2 rounded-xl px-4 py-2 cursor-pointer ${
              segmentTypes === "Box" ? "outline-blue-700 outline outline-[2.5px]" : "outline outline-gray-200"
            } ${isCutOut && "hidden"}`}
            onMouseEnter={() => {
              clearTimeout(boxTimeout);
          ...
          >
            <div className="flex">
              ...
              <span className={`pl-2 font-bold ${segmentTypes === "Box" && "text-blue-600"}`}>Box</span>
            </div>
            {segmentTypes !== "Box" && visibleBoxHover && (
              <Animate isMounted={isBoxMounted}>
                <p className="my-3 text-xs opacity-70">Roughly draw a box around an object.</p>
              </Animate>
            )}
            {segmentTypes === "Box" && <p className={`my-3 text-xs text-blue-700 opacity-70`}>Roughly draw a box around an object.</p>}
            <div className="flex justify-between mx-5 my-3">
              <div onClick={() => setUserNegClickBool(false)} className="flex flex-col items-center">
                <p
                  className={`w-8 h-7 text-3xl leading-7 text-center align-middle rounded-lg mb-1 ${
          ...
          <div
            ...
            className={`transition-all overflow-hidden ${segmentTypes === "All" && isAllAnimationDone === false && "disabled"} ${
              segmentTypes !== "All" && (isAllCollapsed ? "max-h-[40px]" : "max-h-[85px]")
            } my-2 rounded-xl px-4 py-2 cursor-pointer ${
              segmentTypes === "All" ? "outline-blue-700 outline outline-[2.5px]" : "outline outline-gray-200"
            } ${(!isModelLoaded["allModel"] || (isLoading && !isErased)) && "pointer-events-none"} ${isCutOut && "hidden"}`}
            onMouseEnter={() => {
              clearTimeout(allTimeout);
          ...
          >
            <div className="flex">
              ...
              <span
                className={`pl-3 font-bold ${segmentTypes === "All" && "text-blue-600"} ${
                  (!isModelLoaded["allModel"] || (isLoading && !isErased)) && "disabled"
                }`}
              >
                Everything
              </span>
            ...
            {segmentTypes !== "All" && visibleAllHover && (
              <Animate isMounted={isAllMounted}>
                <p className={`text-xs my-3 opacity-70 ${(!isModelLoaded["allModel"] || (isLoading && !isErased)) && "disabled"}`}>
                  Find all the objects in the image automatically.
                </p>
              </Animate>
            )}
            {segmentTypes === "All" && (
              <p className={`text-xs my-3 opacity-70 text-blue-700 ${(!isModelLoaded["allModel"] || (isLoading && !isErased)) && "disabled"}`}>
                Find all the objects in the image automatically.
              </p>
            )}
            {segmentTypes === "All" && (
          ...
          <div
            ...
            className={`transition-all overflow-hidden my-2 rounded-xl px-4 py-2 cursor-pointer ${
              isCutOut ? "outline-blue-700 outline outline-[2.5px]" : "outline outline-gray-200"
            }`}
            onMouseEnter={() => {
              clearTimeout(cutOutTimeout);
          ...
          >
            <div className={`flex ${isCutOut === false && (isCutOutCollapsed ? "max-h-[40px]" : "max-h-[2048px]")}`}>
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="mt-1">
                <path
              ...
              <span className={`pl-2 font-bold ${isCutOut && "text-blue-600"}`}>Cut-Outs</span>
              {isCutOut && (
                <button
