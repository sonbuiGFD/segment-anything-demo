...
export default function Gallery({
  ...
  const chunks = isSingle ? [] : flowVertical ? chunkArray(children, cols) : zipArray(chunkArray(children, React.Children.count(children) / cols));

  return (
    <div className={`comp_gallery flex w-screen justify-center self-stretch ${bgColor} ${textColor}`} style={style}>
      <div className={`max-w-screen-xl flex flex-1 flex-row items-center justify-start px-5 md:px-20 xl:px-10 ${padTop} ${padBottom}`}>
        {!fullWidth && (
        ...
        <div className={`flex flex-col md:flex-row flex-1 flex-grow-4 self-start max-w-none ${fullWidth ? "" : "mx-4"}`}>
          {!isSingle &&
            chunks.map((chs, k) => (
              <div className="flex-col flex-1" key={`group${k}`}>
                {chs.map((c: any, i: number) => (
                  <div className={`p-1 md:p-3 ${grid ? "aspect-[1/1]" : "aspect-auto"}`} key={`img${i}`}>
                    {c}
                  </div>
                ))}
              </div>
