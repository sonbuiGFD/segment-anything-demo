        ...
        <div className="modal modal-open">
          <div className="flex flex-col items-center justify-center h-72 modal-box">
            ...
            <p className="py-4 text-sm md:text-lg">
              {isErasing && eraserText.isEmbedding && "Re-extracting embedding on the erased image"}
              {isErasing && eraserText.isErase && "Masks can be fed into other open source models"}
              {!isErasing && "Extracting an embedding for the image..."}
            </p>
            <div className="loading-bar"></div>
            <button className="pt-8 text-lg font-bold" onClick={handleResetState}>
              Cancel
            </button>
          </div>
        </div>
