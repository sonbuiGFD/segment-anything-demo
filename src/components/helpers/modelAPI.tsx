...
const queryEraseModel = async ({ image, mask, handlePredictedImage }: queryEraseModelProps) => {
...
const setParmsandQueryEraseModel = ({ width, height, uploadScale, imgData, mask, handlePredictedImage }: setParmsandQueryEraseModelProps) => {
...
const isFirstClick = (clicks: Array<modelInputProps>) => {
  return (
    (clicks.length === 1 && (clicks[0].clickType === clickType.POSITIVE || clicks[0].clickType === clickType.NEGATIVE)) ||
    (clicks.length === 2 && clicks.every((c) => c.clickType === clickType.UPPER_LEFT || c.clickType === clickType.BOTTOM_RIGHT))
  );
};

const modelData = ({ clicks, tensor, modelScale, point_coords, point_labels, last_pred_mask }: modeDataProps) => {
  ...
  if (clicks) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      pointCoords[2 * (i + clicksFromBox) + 1] = clicks[i].y / modelScale.onnxScale;
  ...
  const lastPredMaskTensor =
    last_pred_mask && clicks && !isFirstClick(clicks) ? last_pred_mask : new Tensor("float32", new Float32Array(256 * 256), [1, 1, 256, 256]);

  // +!! is javascript shorthand to convert truthy value to 1, falsey value to 0
  const hasLastPredTensor = new Tensor("float32", [+!!(last_pred_mask && clicks && !isFirstClick(clicks))]);
