...
const { generatePolygonSegments, convertSegmentsToSVG } = require("./custom_tracer");
/**
 ...
 */
export function filterSmallSVGRegions(input: string[], maxRegionSize: number = 100) {
  const filtered_regions = input.filter((region: string) => Math.abs(areaOfSVGPolygon(region)) > maxRegionSize);
...
export function keepArrayForMultiMask(uncertainIoUs: number[], overlapIoUs: number[], uncertainThresh: number = 0.8, overlapThresh: number = 0.9) {
