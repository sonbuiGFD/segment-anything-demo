...
const createCutouts = async (
  ...
  const doCreateCutouts = () => {
    ...
    if (segmentTypes === "All" && allsvg) {
      ...
      const stickerSvgs = isMobile ? allsvg?.slice(0, MOBILE_CUTOUT_LIMIT) : allsvg;
      ...
      const filteredStickers: HTMLCanvasElement[] = newStickers.filter(
        (sticker: HTMLCanvasElement | undefined): sticker is HTMLCanvasElement => sticker !== undefined
      );
  ...
  const cropImageByPath = (image: HTMLImageElement, pathData: string): HTMLCanvasElement | undefined => {
  ...
  const parsePathData = (pathData: string, scaleX: number, scaleY: number): Point[] => {
  ...
  const cropCanvasToBoundingBox = (canvas: HTMLCanvasElement, x: number, y: number, width: number, height: number): HTMLCanvasElement | undefined => {
