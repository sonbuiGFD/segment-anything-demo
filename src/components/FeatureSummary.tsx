...
export default function FeatureSummary({
  ...
  const linkBody = (l: Action) => (
    <>
      ...
      <span className={`font-bold ${primary} ${darkMode ? "group-hover:text-blue-200" : "group-hover:text-primary"}`}>{l.action}</span>
    </>
  );

  return (
    <div className={`comp_summary ${darkMode ? "dark-mode" : ""} ${centerAlign ? "text-center" : "text-left"} ${className}`}>
      {label && (small ? <p className={`${secondary} mb-0`}>{label}</p> : <h6 className={`${secondary} mb-2`}>{label}</h6>)}

      <div>{children}</div>

      <div className={`flex gap-5 ${centerAlign ? "items-center" : "items-start w-full"} ${justifyCenter && "justify-center"}`}>
        {/* This button should be refactored as its own component */}
        {actions &&
          actions.map((l) => {
            return useNavLink ? (
              <NavLink to={l.actionUrl} className="flex items-center gap-2 py-2 mt-4 no-underline group">
                {linkBody(l)}
              </NavLink>
            ) : (
              <a href={l.actionUrl} className="flex items-center gap-2 py-2 mt-4 no-underline group">
                {linkBody(l)}
              </a>
            );
          })}
