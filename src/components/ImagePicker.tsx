...
const ImagePicker = ({ handleSelectedImage, showGallery: [showGallery, setShowGallery] }: ImagePickerProps) => {
  ...
  const downloadAllImageResponses = () => {
    photos.forEach((photo, i) => {
      setTimeout(() => {
        handleSelectedImage(new URL(photo.src, window.location.origin), {
          shouldDownload: true,
        });
      }, i * 30000);
    });
  ...
  const StarterModal = () => {
    return (
      <div className="fixed top-0 bottom-0 left-0 right-0 flex items-center justify-center w-full h-full z-1000 bg-gray-300/90" id="modal-terms">
        <div className="modal-box align-middle h-min shadow-[0px_0px_20px_10px_#00000024]">
          ...
          <ul className="px-6 pt-6 list-disc">
            <li className="p-1">
              This is a <strong>research demo </strong>and may not be used for any commercial purpose
            </li>
            <li className="p-1">
              Any images uploaded will be used solely to demonstrate the Segment Anything Model. All images and any data derived from them will be
              deleted at the end of the session.
            </li>
            <li className="p-1">Any images uploaded should not violate any intellectual property rights or Facebook's Community Standards.</li>
          </ul>

          <div className="flex flex-row m-4 md:mt-6 md:mb-6">
            <Checkbox className="mt-1 mr-2" checked={acceptedTerms} onChange={() => handleAttemptContinue()} />
            <div>
              <div className="pb-4">
                <span>
    ...
    <div className="pt-6 mx-4">
      ...
      <div className="flex flex-row py-5 text-sm align-middle md:text-lg">
        ...
        <div className="flex items-center">
          <svg width="8" height="10" viewBox="0 0 8 10" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-2">
            <path d="M8 6.32142L4 10L0 6.32142L0.69323 5.67114L3.5 8.25215L3.5 0H4.5L4.5 8.23964L7.29289 5.67114L8 6.32142Z" fill="#1C2B33" />
          </svg>
          <span>Find a photo in the gallery, or</span>
          <span {...getRootProps()}>
            <input {...getInputProps()} />
            <button className="ml-1 text-blue-700 underline">Upload an image</button>
          </span>
        </div>
      </div>
      <div className={`h-full w-full overflow-y-scroll pb-20 ${showGallery ? "fade-in" : ""}`}>
        <PhotoAlbum
