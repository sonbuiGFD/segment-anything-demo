...
const ToolTip = ({ hasClicked, annotations, isHoverToolTip: [isHoverToolTip, setIsHoverToolTip], allText: [allText, setAllText] }: ToolTipProps) => {
  const {
  ...
  const getText = () => {
    ...
    if (isMultiMaskMode) {
      if (clicks?.length) return "Move your cursor on or off the image to expand or collapse the layers.";
      return "SAM predicts multiple mask possibilities with a single click. Select an object to start.";
    }
    if (segmentTypes === "Click") {
      if (isMobile) {
        if (hasClicked && clicks?.length) return "Cut out the selected object using the Cut-out tool.";
        return "Select any object, SAM is running in the browser.";
      }
      if (hasClicked && clicks?.length) return "Cut out the selected object, or try multi-mask mode.";
      if (isHoverToolTip) return "When hovering over the image, SAM is running in the browser.";
    }
    if (segmentTypes === "Box") {
  ...
  return (
    <>
      <div className={`z-40 flex md:h-8 h-14 mt-2 md:mt-6 w-11/12 items-center top-[7%] md:w-full justify-center ${!!getText() || "invisible"}`}>
        <div className="flex gap-1 p-2 bg-blue-200 rounded-lg w-fit">{getText()}</div>
      </div>
    </>
  );
};
