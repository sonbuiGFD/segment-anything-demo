    ...
    <>
      ...
      >
        <button
          ...
          className={`${((!svg && !svgs && !isErased) || segmentTypes === "All") && "disabled"}`}
        >
          Reset
        </button>
        <button onClick={handleUndoInteraction} className={`${(!svg || segmentTypes === "All" || isMultiMaskMode) && "disabled"}`}>
          Undo
        </button>
        <button onClick={handleRedoInteraction} className={`${(!clicksHistory?.length || segmentTypes === "All") && "disabled"}`}>
          Redo
        </button>
      </div>
      <div
        className={`flex flex-col gap-3 py-3 pl-3 text-sm bg-gray-200 cursor-pointer rounded-xl ${
          hasClicked && "bg-gradient-to-r from-gray-200 to-blue-400/30 background-animate"
        } ${isCutOut && "hidden"}`}
      >
        {segmentTypes === "Click" && (
          <button
          ...
          >
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect x="1" y="1" width="12" height="12" rx="2" fill="#DEE3E9" stroke={isMultiMaskMode ? "#2962D9" : "#1C2A33"} strokeWidth="2" />
              <rect x="4" y="4" width="12" height="12" rx="2" fill="#DEE3E9" stroke={isMultiMaskMode ? "#2962D9" : "#1C2A33"} strokeWidth="2" />
              <rect x="7" y="7" width="12" height="12" rx="2" fill="#DEE3E9" stroke={isMultiMaskMode ? "#2962D9" : "#1C2A33"} strokeWidth="2" />
            </svg>

            <span className={`pl-2 opacity-70 font-bold ${isMultiMaskMode && "text-blue-600 "}`}>Multi-mask</span>
          </button>
        )}
        {/* <button
        ...
        >
          ...
          <span className="pl-2 opacity-70">{segmentTypes === "All" ? "Cut out all objects" : "Cut out object"}</span>
        </button>
      </div>
    </>
  );
