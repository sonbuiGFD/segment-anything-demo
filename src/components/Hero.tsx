...
export default function Hero({
  ...
  const height = fullHeight ? `${h} ${split || overlay ? "" : "md:min-h-[1000px]"}` : "";
  ...
  return (
    <div className={`relative flex flex-col w-screen justify-center items-center ${bg} ${height} px-10 xl:px-0`}>
      <div className={`absolute inset-0 z-0 overflow-hidden ${imageOverlay} ${className}`} style={containerStyle}></div>
      {overlay && video && (
      ...
      <div className={`relative z-1 flex ${contentFlow} gap-10 h-full lg:w-full max-w-screen-xl ${contentAlign} items-center `}>
        <div className={`md:w-[50%] lg:ml-8 ${equalSize} ${contentMargin} ${summaryMargin}`}>{children}</div>

        {split && (
          <div className={`flex ${imageW} ${imageH} ${equalSize} items-center relative`}>
            {image && !video && <div style={{ backgroundImage: `url(${image})` }} className="h-full w-full bg-cover bg-center bg-no-repeat" />}

            {video && <Video url={video} autoPlay loop muted poster={image || undefined} controls={false} aspectRatio="fill" />}
          </div>
        )}
      </div>
    </div>
